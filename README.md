...
1. 仅加入队伍和创建队伍的人可以看见队伍操作按钮 （listTeam 接口要能获取我加入的队伍状态）
2. 强制登录，前端全局响应拦截，自动跳转到登录页
3. 加入有密码的房间要设置密码
4. 查看个人创建的队伍列表，修改队伍信息
5. 前端死【标题】问题
6. 展示队伍已加入人数
7. 分布式锁 导致的其他服务器数据不统一的问题
   或者多个 Redis 里的数据不一致。
8. 重复加入队伍的问题（加锁，分布式锁）
9. 用户登录页跳转
10. 标签
11. 仅加入队伍和创建队伍的人能看到队伍
12. 可以增加AOP进行全局的参数判空操作，这样conller中就可以不需要重复判空了
